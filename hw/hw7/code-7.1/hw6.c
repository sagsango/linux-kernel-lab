...
struct task_count {
    ...
    unsigned long count;             /* Number of times scheduled */
...
static int __kprobes handler_pre(struct kprobe *p, struct pt_regs *regs)
{
    ...
    bool found = false;
    unsigned long irqs;
    /* XXX: prev task from rsi, safe to read as a pointer in this atomic context (preemption disabled by kprobe) */
    struct task_struct *task = (struct task_struct *)regs->si;

    if (!task) {
        printk(KERN_INFO "hw6: prev task is NULL\n");
        return 0;
    }

    /* Capture kernel stack trace of the current execution context */
    nr_entries = stack_trace_save(stack_entries, MAX_STACK_DEPTH, 1); /* Skip 1 for this handler */
    if (nr_entries == 0) {
        printk(KERN_INFO "hw6: No stack trace captured for prev task %px\n", task);
        return 0;
    }

    /* Hash the stack trace using jhash2 */
    ...
    if (!found) {
        ...
        if (!entry) {
            printk(KERN_ERR "hw6: Memory allocation failed for stack trace (hash %u, task %px)\n", hash_key, task);
        ...
        if (!entry->stack_entries) {
            kfree(entry);
            printk(KERN_ERR "hw6: Stack entries allocation failed for hash %u, task %px\n", hash_key, task);
            spin_unlock_irqrestore(&hash_lock, irqs);
            return 0;
        }
        ...
        printk(KERN_INFO "hw6: Added new stack trace with hash %u for prev task %px\n", hash_key, task);
    }
    spin_unlock_irqrestore(&hash_lock, irqs);
...
static int perftop_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "Stack Trace Hash\tScheduled Count\tStack Trace\n");

    spin_lock_irqsave(&hash_lock, irqs);
    hash_for_each(task_hash_table, bkt, entry, node) {
        seq_printf(m, "%u\t\t%lu\t\t", entry->hash_key, entry->count);
...
MODULE_DESCRIPTION("Track prev task scheduling counts with stack traces using Kprobes");
