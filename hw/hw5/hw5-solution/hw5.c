...
static int store_value_xarray(int val)
{
	 ...
	 */
	struct xarrentry *entry = kmalloc(sizeof(*entry), GFP_KERNEL);
	if (!entry)
		return -ENOMEM;
	entry->val = val;
	xa_store(&myxarray, val, entry, GFP_KERNEL);
...
static int store_value_rbtree(int val)
{
	 ...
	 * */
	struct rbentry *entry = kmalloc(sizeof(*entry), GFP_KERNEL);
	struct rb_node **link = &myrbtree.rb_node, *parent = NULL;
	...
	while (*link) {
		struct rbentry *this = container_of(*link, struct rbentry, rbnode);
		parent = *link;
		if (val < this->val)
			link = &((*link)->rb_left);
		else
	...
	rb_insert_color(&entry->rbnode, &myrbtree);
...
static int store_value_hash_table(int val)
{
	 ...
	 * */
	 ...
	 struct hentry *entry = kmalloc(sizeof(*entry), GFP_KERNEL);
	 if (!entry)
		 return -ENOMEM;
	 entry->val = val;
...
static void destroy_rbtree_and_free(void)
{
	 ...
	 * */
	while ((node = rb_first(&myrbtree))) {
		struct rbentry *entry = rb_entry(node, struct rbentry, rbnode);
 ...
 *
 * Opens the proc file and sets up a sequential read operation. 
 * This function is called when a user process opens the /proc/hw5 file. 
 * by calling single_open();
 * 
 * single_open() to initialize a sequential file for reading.
 * This function simplifies handling /proc file reads using seq_file.
 * 
 * API: 
 * int single_open(struct file *file, int (*show)(struct seq_file *, void *), void *data);
 * 
 * Parameters:
 * - file: Pointer to the file structure representing the opened proc file.
 * - show: A callback function (kern_ds_proc_show) that writes output when the file is read.
 * - data: Private data passed to the show function (NULL in this case).
 * 
 * Returns:
 * - 0 on success.
 ...
 * Explain the following code here.
 * 
 * This structure defines the operations for a proc file. 
 * It specifies functions that handle file operations
 * open, read, lseek & release. (As /proc is not a real FS, but we still
 * need to suport basic file operations.)
 * */
...
static int __init hw5_init(void)
{
	 ...
	 * Explain the following code here.
	 * 
	 * Creates a proc file named "hw5" under /proc. 
	 * 
	 * API: 
	 ...
	 * 
	 * Parameters:
	 * - name: The name of the proc file to create.
	 * - mode: The file permissions (0 means default).
	 * - parent: The parent directory (NULL creates it under /proc).
	 * - proc_ops: Pointer to a struct defining the file operations.
	 * 
	 * Returns:
	 * - A pointer to the proc entry on success.
	 * - NULL on failure.
	 * */
...
static void __exit hw5_exit(void)
{
	 ...
	 * Explain the following code here.
	 *
	 * Removes the proc file named "hw5" from /proc. 
	 * During module cleanup to unregister 
	 * the proc file and free associated resources. 
	 * 
	 * API: 
	 * void remove_proc_entry(const char *name, struct proc_dir_entry *parent);
	 * 
	 * Parameters:
	 * - name: The name of the proc file to remove.
	 * - parent: The parent directory (NULL means it's in /proc).
	 * 
	 * Effects:
	 * - Deletes /proc/hw5, making it inaccessible.
	 * - If the proc file is still in use, it will be removed once all references are closed.
	 * */
